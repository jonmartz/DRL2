...
with tf.Session() as sess:
    ...
    for episode in range(max_episodes):
        ...
        for t, tr in enumerate(episode_transitions):
            ...
            feed_dict = {baseline_net.state: tr.state, baseline_net.value: total_discounted_return,
                         baseline_net.advantage: advantage}
            _, baseline_loss = sess.run([baseline_net.optimizer, baseline_net.loss], feed_dict)
            ep_baseline_losses.append(baseline_loss)
            feed_dict = {policy_net.state: tr.state, policy_net.target: advantage, policy_net.action: tr.action}
